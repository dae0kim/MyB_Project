...
<c:choose>
  	...
  	<c:when test="${loginUser.adminLevel eq 'principal'}">
  		<%@include file="../principal/staticFiles.jsp" %>
  	</c:when>
  </c:choose>
<body>
    <div class="preloader">
	...
	$(function () {
	    ...
	    for (var state in aus) {
	      (function (st, state) {
	      	...
	      	/* 색상 배분 */
	      	if(locInfo[state][4] >= 10){
	      		$(st.node).attr("fill", "#6e020f");
	      	}else if(locInfo[state][4] >= 7 && locInfo[state][4] < 10){
	      		$(st.node).attr("fill", "#a91314");
	      	}else if(locInfo[state][4] >= 5 && locInfo[state][4] < 7){
	      		$(st.node).attr("fill", "#f96849");
	      	}else if(locInfo[state][4] >=2 && locInfo[state][4] < 5){
	      		$(st.node).attr("fill", "#fcbb9f");
	      	}else{
	      		$(st.node).attr("fill", "#fbdecc");
	      	}
	...
	function redraw(url){
	    ...
	    function drawChart() {
	  	  ...
	  	  var jsonData = $.ajax({
	            url: "http://localhost:8080/MyB_Project/diseaselog/chart/json/"+url,
	            dataType:"json",
	            async: false
	            }).responseText;
